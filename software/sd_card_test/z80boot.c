...
int sdmbrentry(unsigned char *partptr)
    ...
    if ((curblkno != 0) || !curblkok)
        {
        curblkno = 0;
        if (!sdread(sdrdbuf, curblkno))
            {
...
/* read MBR partition information */
void sdmbrpart()
    {
    ...
    entp = &sdrdbuf[0x01be];
    for (partidx = 1; partidx <= 4; partidx++, entp += 16)
        {
#ifdef SDTEST
        printf("MBR partition entry %d: ", partidx);
#endif
        if (!sdmbrentry(entp))
            break;
        }
